...
import 'package:wm_com/src/global/api/restaurant/facture_rest_api.dart';
...
class FactureRestaurantController extends GetxController
  ...
  final FactureRestApi factureRestApi =
      FactureRestApi();
  ...
  void syncData() async {
    try {
      _isLoading.value = true;
      var dataCloudList = await factureRestApi.getAllData();
      ...
      if (dataCloudList.isEmpty) {
        if (dataList.isNotEmpty) {
          for (var element in dataList) {
            ...
            await factureRestApi.insertData(dataItem).then((value) async {
              ...
              await factureRestaurantStore.updateData(dataItem).then((value) {
                factureList.clear();
                getList();
                if (kDebugMode) {
            ...
            if (dataUpdateList.isNotEmpty) {
              for (var element in dataUpdateList) {
                ...
                if (e.created.millisecondsSinceEpoch ==
                  ...
                  await factureRestApi.updateData(dataItem).then((value) async {
                    FactureRestaurantModel dataModel = dataList
                        .where((p0) =>
                            p0.created.millisecondsSinceEpoch ==
                            value.created.millisecondsSinceEpoch)
