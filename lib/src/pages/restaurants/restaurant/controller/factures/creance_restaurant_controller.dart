...
import 'package:wm_com/src/global/api/restaurant/creance_rest_api.dart';
import 'package:wm_com/src/global/store/restaurant/creance_restaurant_store.dart';
import 'package:wm_com/src/global/store/restaurant/facture_restaurant_store.dart';
...
class CreanceRestaurantController extends GetxController
  ...
  final CreanceRestApi creanceRestApi =
      CreanceRestApi();
  ...
  void syncData() async {
    try {
      _isLoading.value = true;
      var dataCloudList = await creanceRestApi.getAllData();
      var dataList = creanceFactureList.where((p0) => p0.sync == "new").toList();
      var dataUpdateList = creanceFactureList.where((p0) => p0.sync == "update").toList();
      if (dataCloudList.isEmpty) {
        if (dataList.isNotEmpty) {
          for (var element in dataList) {
            ...
            await creanceRestApi.insertData(dataItem).then((value) async {
              ...
              await creanceRestaurantStore.updateData(dataItem).then((value) {
                creanceFactureList.clear();
                getList();
                if (kDebugMode) {
        ...
        if (creanceFactureList.isEmpty) {
          for (var element in dataCloudList) {
            ...
            await creanceRestaurantStore.insertData(dataItem).then((value) {
              if (kDebugMode) {
            ...
            if (dataUpdateList.isNotEmpty) {
              for (var element in dataUpdateList) {
                ...
                if (e.created.millisecondsSinceEpoch ==
                  ...
                  await creanceRestApi.updateData(dataItem).then((value) async {
                    CreanceRestaurantModel dataModel = dataList
                  .where((p0) =>
                      p0.created.millisecondsSinceEpoch ==
                      value.created.millisecondsSinceEpoch)
